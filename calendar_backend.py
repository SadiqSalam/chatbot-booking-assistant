...
def ask():
    ...
    try:
        ...
        intent = classify_query_intent_cached(query)
        query_is_chinese = is_chinese(query)
        timer.mark("after_intent_and_chinese_check")
        ...
        if intent == "greeting":
            ...
            for item in parsed_list:
                try:
                    try:
                        req_start = datetime.fromisoformat(item["start"])  # or item["start"]
            ...
            if max_end_dt is None:
                # fallback to 1 day ahead if parsing failed
                max_end_dt = start_of_today + timedelta(days=1)

            # Add 30 min buffer for event fetching
            fetch_end_dt = max_end_dt + timedelta(minutes=30)
            
            # --- NEW: Collect all calendar IDs for all rooms involved in parsed_list ---
            all_rooms = set()
            for item in parsed_list:
            ...
            # --- NEW: Batch fetch events only from today to max end datetime + buffer ---
            cached_events = {}
            if calendar_ids:
